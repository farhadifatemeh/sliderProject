...
<body>	
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<!--<div><br/></div>
	<div class="container"><div class="inline" ><lable id="lablescore" class="col-xs-6 col-md-4 ">Score=0</lable></div>
	<div class=" inline" ><lable id="lablehighscore" class="col-xs-6 col-md-4 ">High Score=0</lable></div><div> <h5> <br/></h5></div></div>-->
	<div><br/></div>
	<div class="container" ><lable id="lablescore" class="col-xs-6 col-md-4 ">Score=0</lable>
	<lable id="lablehighscore" class="col-xs-6 col-md-4 ">High Score=0</lable> <h5> <br/></h5></div>
...
<div id="content" class="div container">
   ...
   <button  type="button" id="add" class="btn active btn-sm"><span class="glyphicon glyphicon-plus"></span>Add</button>
	...
	<div><br/> <br/></div>
<div class="container" >
	<lable id="lablehighalert" class="col-xs-6 col-md-4 ">alert</lable> <h5> <br/></h5></div>
	<div id="bottom1 row">
...
function subscriber(mutations) {
  mutations.forEach((mutation) => {
   ...
   // alert(mutation);
	  var strscore="";
	  $.ajax({
    method: "GET", url: "getmaxscore980602.php",  //dataType:"json",

  }).done(function( data) {

   var resultscore = jQuery.parseJSON(data);
		//alert(resultscore[0]['total']); 
			$("#lablehighscore").text("High Score= "+resultscore[0]['total']);
							 for(var i=0;i<resultscore[0]['userid'].length;i++){
							strscore=resultscore[1]['total'][i];
								 
							 }
		  
								// $("#lablehighscore").text("High Score= "+strscore);
	  });
	  $( "#identy" ).empty();
	  		score=0;	
		    var j=0;
	   ...
	   var flagstr="";
			$('.owl-carousel').each(function(index){
				...
				var item2=$('.owl-item.active').eq(((k)*22)+i);
					...
					if(flag==0){
						var tempreverse1=$("#content .divrow").find('.isreversediv .isreverselable').eq(index).text();
						var tempreverse2=$("#content .divrow").find('.isreverselable').eq(k).text();
						if((tempreverse1=='yes')||(tempreverse2=='yes')){
							flagrev=flagrev+'yes';
							$("#lablehighalert").text(tempreverse1+tempreverse2+"yes");
						}
						else {
							flagrev=flagrev+'no';
							$("#lablehighalert").text(tempreverse1+tempreverse2+"no");
						}
					...
					var tempstr1="";
				        ...
				        for(var i=0;i<arraypattern.length-1;i++){
							...
							if(arraypattern[i].length>1&& t==1&&arraypattern[i].length<8){
			...
			$('#identy .owl-carousel').each(function(index){
				...
				if(arraypattern[i]==c){
					...
					if(reverse[i]=='yes'){$('#identy .owl-carousel > .flagreverseh').eq(index).text('yes');}
					fl=0;
				}
			});
						if(fl==1){
			...
			$(html2).append($("<h5 style='font-size:1vw' class='scorepattern inline'</h5>").text(tempstr1).css("color","#000000")).append($("<h5 style='font-size:1vw'class='pattern inline'</h5>").text(arraypattern[i]).css("color","#FFFFFF")).append($("<h5 style='font-size:1vw'class='flagpattern inline'</h5>").text(arrayflag[i]).css("color","#000000")).append($("<h5 style='font-size:1vw'class='flagreverseh inline'</h5>").text(reverse[i]).css("color","#000000")).append($("<button type='button' class='btn  btn-sm approvepattern' onclick='approvepattern()'>").append("<span class='glyphicon glyphicon-ok'></span>")).append($("<button type='button' class='btn  btn-sm removepattern' onclick='removepattern()'>").append("<span class='glyphicon glyphicon-remove'></span>"));
							//alert(reverse);
								$('#identy').append(html2);
						}
									}
...
observer.observe(target, config);
   ...
   var result = jQuery.parseJSON(data);
		 ...
		 //str=strsequence;
           ...
           html = $("<div class='owl-carousel owl-theme row'>");
				        for(var i=0;i<15;i++){
        ...
        for(var i=0;i<15;i++){
           ...
           .append("<h5>X</h5>").css("color","#FFFFFF"));
					...
					$(html1).append($("<div class=' mydiv'>").append("<button type='button' class='btn btn-sm' onclick='delete1()'>x</button>"));
					$(html1).append($("<div class='slider'>").append(html));
					$(html1).append($("<div class='reversediv'>").append("<button type='button' class='btn btn-sm' onclick='reverseslider()'>Reverse</button>"));
					$(html1).append($("<div class='isreversediv'>").append("<lable type='lable' class='isreverselable' >no</lable>").css("color","#000000"));
...
function reverseslider(){
		$("#content .divrow").click(function(event){
			if(event.handled !== true){
				//var itemrow=[];
				var tempstr="";
				var par=$("#content .divrow").index(this);
				var par1=$(this).closest('.divrow').find('.owl-carousel').eq(0);
		        var revflag=$(this).closest('.divrow').find('.isreversediv .isreverselable').eq(0).text();
				alert(revflag);
				var lengthp=par1.find('.owl-item.active .item').length;
				for(var i=0;i<lengthp;i++){
					var itemrow=par1.find('.owl-item.active .item').eq(i).text();
					tempstr=tempstr+itemrow;
				}
				var str=tempstr.split("").reverse().join("");
				var html1=$("<div class='divrow row'>");
           html = $("<div class='owl-carousel owl-theme row'>");
				        for(var i=0;i<15;i++){
			$(html).append($("<div class='item col-xs-6 col-md-4'>").css("background-color","#FFFFFF")
        ...
        for(var i=0;i<15;i++){
           ...
           .append("<h5>X</h5>").css("color","#FFFFFF"));
					...
					$(html1).append($("<div class=' mydiv'>").append("<button type='button' class='btn btn-sm' onclick='delete1()'>x</button>"));
					$(html1).append($("<div class='slider'>").append(html));
					$(html1).append($("<div class='reversediv'>").append("<button type='button' class='btn btn-sm ' onclick='reverseslider()'>y</button>"));
				if(revflag=='no'){
					$(html1).append($("<div class='isreversediv'>").append("<lable type='lable' class='isreverselable' >yes</lable>").css("color","#000000"));
				}
				else if(revflag=='yes'){
					$(html1).append($("<div class='isreversediv'>").append("<lable type='lable' class='isreverselable' >no</lable>").css("color","#000000"));
				}
    ...
    // $("#content").append(html1);
				var p=$(this).closest('.divrow').eq(0);
				p.replaceWith(html1);
				//alert(p);
				//p.remove();
			$(html).owlCarousel({
		...
		function delete1() {
			$("#content .mydiv").click(function(event){
				if(event.handled !== true){
				//alert("bh");
					var par=$(this).parent();
					par.remove();
